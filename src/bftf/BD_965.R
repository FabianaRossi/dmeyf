...
dataset  <- fread( "./datasets/dataset_epic_v962.csv.gz")
...
dataset_aux  <- fread( "./datasetsOri/paquete_premium.csv.gz")
vars_a_sacar <- c('numero_de_cliente', 'clase_ternaria','foto_mes')
dataset_aux[  , c(vars_a_sacar) := NULL ]

dataset <- cbind(dataset,dataset_aux)
